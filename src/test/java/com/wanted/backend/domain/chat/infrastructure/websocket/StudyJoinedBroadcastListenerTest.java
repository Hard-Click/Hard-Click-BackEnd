...
class StudyJoinedBroadcastListenerTest {

    @InjectMocks
    private StudyJoinedBroadcastListener listener;
    ...
    void handle_success() {
        // given
        StudyJoinedEvent event = new StudyJoinedEvent(12L, 45L, 2L, 4);
        given(memberNamePort.getNamesByMemberIds(any())).willReturn(Map.of(2L, "김민수"));
        List<ParticipantPresenceMessage> participants = List.of(
        ...
        assertThat(message.participantCount()).isEqualTo(2);
        ...
        assertThat(messageCaptor.getValue().getType()).isEqualTo(ChatMessageType.SYSTEM_JOIN);
        assertThat(messageCaptor.getValue().getContent()).isEqualTo("김*수님이 입장했습니다");
    }
