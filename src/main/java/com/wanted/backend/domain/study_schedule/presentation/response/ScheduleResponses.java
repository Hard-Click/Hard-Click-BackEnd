...
public final class ScheduleResponses {
    ...
    @Schema(description = "스케줄 캘린더 항목. AI가 배치한 강의(LESSON)와 학생이 추가한 할 일(TODO)이 섞여 내려온다.")
    public record CalendarItemResponse(
            @Schema(description = "항목 출처. LESSON=AI 배치 강의, TODO=학생이 추가한 할 일. "
                    + "완료 처리 엔드포인트가 이 값에 따라 갈린다.", example = "LESSON")
            ...
            @Schema(description = "수강 ID. source=TODO 면 null", example = "1063") Long enrollmentId,
            @Schema(description = "강의 ID. source=TODO 면 null", example = "42") Long courseId,
            @Schema(description = "강의명. source=TODO 면 null", example = "수능 영어 실전") String courseTitle,
